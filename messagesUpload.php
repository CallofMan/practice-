...
for($i = 0; $i < $lengthArray; $i++)
{
    ...
    if($_SESSION['role'] == 1)
    {
        ...
        {
            echo "<div class='message_block' id='message" . $messagesArray[$i][0] . "' style='align-self: flex-end;'><h4>" . $nameUserM['first_name'] . " " . $nameUserM['second_name'] . "</h4> <p class='message_content' style='background-color: rgba(0, 169, 251, 0.3); color:white; opacity: 1;'> " . $messagesArray[$i][2] . " </p> <div class='deleteAndDate'> <p class='delete_message' id='" . $messagesArray[$i][0] . "'>удалить</p> <h6>" . $messagesArray[$i][3] . "</h6></div> </div>";
        }
        else
        {
            echo "<div class='message_block' id='message" . $messagesArray[$i][0] . "' style='align-self: flex-start;'><h4>" . $nameUserM['first_name'] . " " . $nameUserM['second_name'] . "</h4> <p class='message_content'> " . $messagesArray[$i][2] . " </p> <div class='deleteAndDate'> <p class='delete_message' id='" . $messagesArray[$i][0] . "'>удалить</p> <h6>" . $messagesArray[$i][3] . "</h6></div> </div>";
        }
    }
    else
    {
